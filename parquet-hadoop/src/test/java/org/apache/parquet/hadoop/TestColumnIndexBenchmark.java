...
public class TestColumnIndexBenchmark {
  private static final Logger LOGGER = LoggerFactory.getLogger(TestColumnIndexBenchmark.class);
  ...
  @Test
  public void testSimpleFiltering2() throws IOException {
    Path file = new Path("/Users/jiatao.tao/Documents/par-in/001.snappy.parquet");

    String schemaString = "message spark_schema {\n" +
      "  required int64 id;\n" +
    ...
    ParquetReader<Group> reader = ParquetReader.builder(new GroupReadSupport(), file)
      .withFilter(FilterCompat.get(eq(longColumn("id"), 1234L)))
      //.useDictionaryFilter(false)
      //.useStatsFilter(false)
      //.useRecordFilter(false)
      .useColumnIndexFilter(true)
      .set(GroupWriteSupport.PARQUET_EXAMPLE_SCHEMA, schema.toString()).build();

    for (Group group = reader.read(); group != null; group = reader.read()) {
      int integer = group.getInteger(1, 0);
